...
export async function initializer(Environment)  {
{
    ...
    if (check_true (await (await __GG__("not"))(await (await __GG__("not"))(((typeof "Deno"==="undefined")||(await Environment["get_global"].call(Environment,"Deno") instanceof ReferenceError))))))throw new Error("IO requires Deno");
    ;
    await Environment.set_global("read_text_file",await (await __GG__("bind"))((await __GG__("Deno.readTextFile")),(await __GG__("Deno"))),{
    ...
    await Environment.set_global("load",async function(filename) {
        ...
        fname=filename;
        js_mod=null;
        comps=await (await __GG__("path.parse"))(fname);
         return  await async function(){
            if (check_true( ((comps && comps["ext"])===".lisp"))) {
                 return await (await __GG__("evaluate"))(await (await __GG__("read_text_file"))(fname),null,{
                    source_name:fname
                })
                ...
                if (check_true ((js_mod && js_mod["initializer"]))){
                 ...
                 return await (await __GG__("evaluate"))(await JSON.parse(await (await __GG__("read_text_file"))(fname)),null,{
                    json_in:true
                })
            }
        } ()
...
await Environment.set_global("write_text_file",await (await __GG__("bind"))((await __GG__("Deno.writeTextFile")),(await __GG__("Deno"))),{
...
await Environment.set_global("compile_file",async function(lisp_file,export_function_name,options) {
    ...
    let boilerplate;
    let compiled_js;
    input_components=await (await __GG__("path.parse"))(lisp_file);
    input_filename=await (await __GG__("path.basename"))(lisp_file);
    output_filename=((options && options["output_file"])||await (await __GG__("add"))(await (async function() {
         if (check_true (((input_components && input_components["dir"])===""))){
              return "."
        } else {
              return (input_components && input_components["dir"])
        } 
    } )(),(await __GG__("path.sep")),(input_components && input_components["name"]),".js"));
    ...
    compiled_js=null;
    if (check_true ((await (await __GG__("length"))(await (await __GG__("scan_str"))(invalid_js_ref_chars_regex,export_function_name))>0))){
    ...
    input_buffer=await (await __GG__("read_text_file"))(lisp_file);
    if (check_true (((input_components && input_components["ext"])===".lisp"))){
         input_buffer=await (await __GG__("read_lisp"))(input_buffer,{
    ...
    compiled=await (await __GG__("compiler"))(input_buffer,await (await __GG__("add"))({
        env:Environment,formatted_output:true,include_source:include_source,source_name:input_filename
    },opts));
    compile_time=await (await __GG__("add"))(await (async function() {
        {
    ...
    await async function(){
        if (check_true((compiled && compiled["error"]))) {
            ...
            write_file=false;
             return  await (await __GG__("warn"))((compiled && compiled["1"]))
        } else if (check_true( ((compiled && compiled["0"] && compiled["0"]["ctype"])&&(await (await __GG__("contains?"))("block",(compiled && compiled["0"] && compiled["0"]["ctype"]))||((compiled && compiled["0"] && compiled["0"]["ctype"])==="assignment")||((compiled && compiled["0"] && compiled["0"]["ctype"])==="__!NOT_FOUND!__"))))) {
    ...
    if (check_true (write_file)){
        await (await __GG__("write_text_file"))(output_filename,(segments).join("\n"));
        await (await __GG__("success"))(("["+compile_time+"] compiled: "),lisp_file,"->",output_filename);
         return  output_filename
    } else {
        await (await __GG__("warn"))("input file ",lisp_file," not compiled.");
         return  null
    }
...
await Environment.set_global("rebuild_env",async function(opts) {
    ...
    dcomps=await (await __GG__("date_components"))(new Date());
    version_tag=await (async function () {
         if (check_true (await (await __GG__("not"))(await (await __GG__("blank?"))((opts && opts["version_tag"]))))){
    ...
    build_time=await (await __GG__("formatted_date"))(new Date());
    build_headers=[];
