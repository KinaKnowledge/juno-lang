...
export async function initializer(Environment)  {
{
    ...
    null;
    {
        ...
        [await Environment.set_global("core/read_text_file",await (await Environment.get_global("bind"))(Deno.readTextFile,Deno),{
            ...
            })(),requires:["bind"],requires:["Deno","EvalError","JSON"],source_name:"base-io.lisp"
        ...
        },{ "name":"load","fn_args":"(filename)","description":["=:+","Compile and load the contents of the specified lisp filename (including path) into the Lisp environment. ","The file contents are expected to be Lisp source code in text format."],"tags":["compile","read","io","file"],"usage":["filename:string"],"requires":["path","evaluate","read_text_file"],"externals":["Deno","EvalError","JSON"],"source_name":"base-io.lisp"
    });
