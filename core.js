...
export async function load_core(Environment) {
    ...
    await Environment.set_global("enum",async function(value_list) {
        ...
        await (async function() {
         ...
         return  e
    },await Environment.do_deferred_splice(await Environment.read_lisp('{\"name\":\"enum\" \"fn_args\":\"(value_list)\" \"usage\":(\"value_list:array\") \"description\":\"Given a list of string values, returns an object with each value in the list corresponding to a numerical value.\" \"tags\":(\"enumeration\" \"values\")}')));
    ...
    await Environment.set_global("macros",async function() {
        ...
        await (async function() {
         ...
         return  __collector
    },await Environment.do_deferred_splice(await Environment.read_lisp('{\"name\":\"macros\" \"fn_args\":\"()\" \"usage\":() \"description\":\"Returns the list of currently defined macros.  This function takes no arguments.\" \"tags\":(\"environment\" \"macro\" \"defined\")}')));
    await Environment.set_global("pluck",async function(fields,data) {
         return  await Environment.do_deferred_splice(await Environment.read_lisp('(each ' + await Environment.as_lisp ( data ) + ' ' + await Environment.as_lisp ( fields ) + ')'))
    ...
    await Environment.set_global("pairs_from_list",async function(value_list,size) {
        ...
        if (check_true ((await (await Environment.get_global("length"))(pset)>0))){
         ...
         return  container
    },await Environment.do_deferred_splice(await Environment.read_lisp('{\"name\":\"pairs_from_list\" \"fn_args\":\"(value_list size)\" \"usage\":(\"value:list\" \"size?:number\") \"description\":\"Given a list, segment the passed list into sub list (default in pairs) or as otherwise specified in the optional size\" \"tags\":(\"list\" \"pairs\" \"collect\")}')));
    ...
    },await Environment.do_deferred_splice(await Environment.read_lisp('{\"name\":\"reorder_keys\" \"fn_args\":\"(key_list obj)\" \"description\":\"Given a list of keys, returns a new object that has the keys in the order of the provided key list.\" \"usage\":(\"key_list:array\" \"obj:object\") \"tags\":(\"list\" \"object\" \"key\" \"order\")}')));
    ...
    },await Environment.do_deferred_splice(await Environment.read_lisp('{\"eval_when\":{\"compile_time\":true} \"name\":\"+=\" \"macro\":true \"fn_args\":\"(symbol \\"&\\" args)\" \"usage\":(\"symbol:*\" \"arg0:*\" \"argn?:*\") \"description\":\"Appends in place the arguments to the symbol, adding the values of the arguments to the end.\" \"tags\":(\"append\" \"mutate\" \"text\" \"add\" \"number\")}')));
    await Environment.set_global("minmax",async function(container) {
        ...
        if (check_true ((container&&(container instanceof Array)&&(await (await Environment.get_global("length"))(container)>0)))){
            ...
            if (check_true (value_found)){
                  return await (async function(){
                    let __array_op_rval__32=smallest;
                     if (__array_op_rval__32 instanceof Function){
                        return await __array_op_rval__32(biggest) 
                    } else {
                        return[__array_op_rval__32,biggest]
                    }
                })()
            } else {
                  return null
    ...
    },await Environment.do_deferred_splice(await Environment.read_lisp('{\"name\":\"minmax\" \"fn_args\":\"(container)\" \"usage\":(\"container:array\") \"description\":\"Given an array of numbers returns an array containing the smallest and the largest values found in the provided array. \" \"tags\":(\"list\" \"number\" \"range\" \"value\")}')));
    await Environment.set_global("minmax_index",async function(container) {
        ...
        if (check_true ((container&&(container instanceof Array)&&(await (await Environment.get_global("length"))(container)>0)))){
            await (async function() {
                let __for_body__35=async function(value) {
                     return  (await (await Environment.get_global("is_number?"))(value)&&await (async function ()  {
                        value_found=true;
                ...
                let __array__36=[],__elements__34=container;
                let __BREAK__FLAG__=false;
            ...
            if (check_true (value_found)){
                  return await (async function(){
                    let __array_op_rval__37=idx_small;
                     if (__array_op_rval__37 instanceof Function){
                        return await __array_op_rval__37(idx_largest) 
                    } else {
                        return[__array_op_rval__37,idx_largest]
                    }
                })()
            } else {
                  return null
    ...
    },await Environment.do_deferred_splice(await Environment.read_lisp('{\"name\":\"minmax_index\" \"fn_args\":\"(container)\" \"usage\":(\"container:array\") \"description\":\"Given an array of numbers returns an array containing the indexes of the smallest and the largest values found in the provided array.\" \"tags\":(\"list\" \"number\" \"range\" \"value\" \"index\")}')));
    await Environment.set_global("invert_pairs",async function(value) {
        if (check_true ((value instanceof Array))){
              return await (await Environment.get_global("map"))(async function(v) {
                 return  await (async function(){
                    let __array_op_rval__38=(v && v["1"]);
                     if (__array_op_rval__38 instanceof Function){
                        return await __array_op_rval__38((v && v["0"])) 
                    } else {
                        return[__array_op_rval__38,(v && v["0"])]
                    }
                })()
            },value)
        } else throw new Error("invert_pairs passed a non-array value");
        
    },await Environment.do_deferred_splice(await Environment.read_lisp('{\"name\":\"invert_pairs\" \"fn_args\":\"(value)\" \"description\":\"Given an array value containing pairs of value, as in [[1 2] [3 4]], invert the positions to be: [[2 1] [4 3]]\" \"usage\":(\"value:array\") \"tags\":(\"array\" \"list\" \"invert\" \"flip\" \"reverse\" \"swap\")}')));
    await Environment.set_global("object_methods",async function(obj) {
        ...
        current_obj=obj;
        await (async function(){
             let __test_condition__39=async function() {
                 return  current_obj
            };
            let __body_ref__40=async function() {
            ...
            let __BREAK__FLAG__=false;
            while(await __test_condition__39()) {
                await __body_ref__40();
                 if(__BREAK__FLAG__) {
                     break;
    ...
    await Environment.set_global("noop",async function(val) {
         return  val
    },await Environment.do_deferred_splice(await Environment.read_lisp('{\"name\":\"noop\" \"fn_args\":\"(val)\" \"usage\":(\"val:*\") \"description\":\"No operation, just returns the value.  To be used as a placeholder operation, such as in apply_operator_list.\" \"tags\":(\"apply\" \"value\")}')));
    await Environment.set_global("apply_list_to_list",async function(operator,list1,list2) {
         return  await (await Environment.get_global("map"))(async function(val,idx) {
             return  await (async function(){
                let __array_op_rval__42=operator;
                 if (__array_op_rval__42 instanceof Function){
                    return await __array_op_rval__42(val,await (async function(){
                        let __targ__41=list1;
                        if (__targ__41){
                             return(__targ__41)[(idx%await (await Environment.get_global("length"))(list1))]
                        } 
                    })()) 
                } else {
                    return[__array_op_rval__42,val,await (async function(){
                        let __targ__41=list1;
                        if (__targ__41){
                             return(__targ__41)[(idx%await (await Environment.get_global("length"))(list1))]
                        } 
    ...
    },await Environment.do_deferred_splice(await Environment.read_lisp('{\"name\":\"form_id\" \"fn_args\":\"(name)\" \"usage\":(\"name:string\") \"description\":\"Given a standard string returns a compliant HTML ID suitable for forms.\"}')));
    ...
    },await Environment.do_deferred_splice(await Environment.read_lisp('{\"name\":\"from_key\" \"fn_args\":\"(value sep? ignore?)\" \"usage\":(\"value:string\" \"separator?:string\") \"description\":(+ \"Takes a key formatted value such as \\"last_name\\" and returns a \\"prettier\\" string that contains spaces \" \"in place of the default separator, \'_\' and each word\'s first letter is capitalized. \" \"An optional separator argument can be provided to use an alternative separator token.<br>E.G. last_name becomes \\"Last Name\\".\") \"tags\":(\"string\" \"split\" \"key\" \"hash\" \"record\" \"form\" \"ui\")}')));
    await Environment.set_global("from_key1",async function(v) {
         return  await (await Environment.get_global("from_key"))(v)
    },await Environment.do_deferred_splice(await Environment.read_lisp('{\"name\":\"from_key1\" \"fn_args\":\"(v)\" \"description\":\"Useful for calling with map, since this function prevents the other values being passed as arguments by map from being passed to the from_key function.\" \"tags\":(\"map\" \"function\" \"key\" \"pretty\" \"ui\" \"to_key\") \"usage\":(\"value:string\")}')));
    ...
    },await Environment.do_deferred_splice(await Environment.read_lisp('{\"name\":\"to_key\" \"fn_args\":\"(value sep? ignore?)\" \"usage\":(\"value:string\" \"separator?:string\") \"description\":(+ \"Takes a value such as \\"Last Name\\" and returns a string that has the spaces removed and the characters replaced \" \"by the default separator, \'_\'.  Each word is converted to lowercase characters as well.\" \"An optional separator argument can be provided to use an alternative separator token.<br>E.G. \\"Last Name\\" becomes \\"last_name\\".\") \"tags\":(\"string\" \"split\" \"key\" \"hash\" \"record\" \"form\" \"ui\")}')));
    await Environment.set_global("is_date?",async function(x) {
         return  (await (await Environment.get_global("sub_type"))(x)==="Date")
    },await Environment.do_deferred_splice(await Environment.read_lisp('{\"name\":\"is_date?\" \"fn_args\":\"(x)\" \"description\":\"for the given value x, returns true if x is a Date object.\" \"usage\":(\"arg:value\") \"tags\":(\"type\" \"condition\" \"subtype\" \"value\" \"what\")}')));
        ...
        if (check_true (((target_object instanceof Object)&&(source_object instanceof Object)))){
            await (async function() {
                let __for_body__49=async function(pset) {
                     return  await async function(){
                        let __target_obj__51=target_object;
                        __target_obj__51[(pset && pset["0"])]=(pset && pset["1"]);
    ...
    },await Environment.do_deferred_splice(await Environment.read_lisp('{\"name\":\"extend\" \"fn_args\":\"(target_object source_object)\" \"description\":\"Given a target object and a source object, add the keys and values of the source object to the target object.\" \"usage\":(\"target_object:object\" \"source_object:object\") \"tags\":(\"object\" \"extension\" \"keys\" \"add\" \"values\")}')));
    ...
    },await Environment.do_deferred_splice(await Environment.read_lisp('{\"name\":\"no_empties\" \"fn_args\":\"(\\"items\\")\" \"description\":\"Takes the passed list or set and returns a new list that doesn\'t contain any undefined, nil or empty values\" \"usage\":(\"items:list|set\") \"tags\":(\"filter\" \"nil\" \"undefined\" \"remove\" \"except_nil\")}')));
    ...
    },await Environment.do_deferred_splice(await Environment.read_lisp('{\"name\":\"first_with\" \"fn_args\":\"(prop_list data_value)\" \"usage\":(\"property_list:array\" \"data:object|array\") \"description\":\"Given a list of properties or indexes and a data value, sequentially looks through the property list and returns the first non-null result.\" \"tags\":(\"list\" \"array\" \"index\" \"properties\" \"search\" \"find\")}')));
    ...
    },await Environment.do_deferred_splice(await Environment.read_lisp('{\"name\":\"fixed\" \"fn_args\":\"(v p)\" \"description\":\"Given a floating point value and an optional precision value, return a string corresponding to the desired precision.  If precision is left out, defaults to 3.\" \"usage\":(\"value:number\" \"precision?:number\") \"tags\":(\"format\" \"conversion\")}')));
    await Environment.set_global("except_nil",async function(items) {
        ...
        await (async function() {
            let __for_body__63=async function(value) {
                if (check_true (await (await Environment.get_global("not"))((null==value)))){
                      return (acc).push(value)
                }
            };
            let __array__64=[],__elements__62=items;
            let __BREAK__FLAG__=false;
    ...
    await Environment.set_global("hide",async function(value) {
         return  undefined
    },await Environment.do_deferred_splice(await Environment.read_lisp('{\"name\":\"hide\" \"fn_args\":\"(value)\"}')));
    await Environment.set_global("array_to_object",async function(input_array) {
        ...
        await (async function(){
                 ...
                 return  await async function(){
                    if (check_true( (v1t==="object"))) {
                         return output=await (await Environment.get_global("add"))(await (async function(){
                            let __array_op_rval__67=output;
                             if (__array_op_rval__67 instanceof Function){
                                return await __array_op_rval__67(v1) 
                            } else {
                                return[__array_op_rval__67,v1]
                            }
                        })())
                    } else  {
                         return await async function(){
                            let __target_obj__68=output;
                            __target_obj__68[v1]=(working_array).shift();
         ...
         return  output
    },await Environment.do_deferred_splice(await Environment.read_lisp('{\"name\":\"array_to_object\" \"fn_args\":\"(input_array)\" \"usage\":(\"list_to_process:array\") \"tags\":(\"list\" \"array\" \"object\" \"convert\") \"description\":\"Takes the provided list and returns an object with the even indexed items as keys and odd indexed items as values.\"}')));
    await Environment.set_global("split_text_in_array",async function(split_element,input_array) {
        let output=[];
        ;
        await (async function() {
            let __for_body__71=async function(item) {
                 return  await async function(){
            ...
            let __array__72=[],__elements__70=input_array;
            let __BREAK__FLAG__=false;
         ...
         return  output
    },await Environment.do_deferred_splice(await Environment.read_lisp('{\"name\":\"split_text_in_array\" \"fn_args\":\"(split_element input_array)\" \"usage\":(\"split_element:text\" \"input_array:array\") \"tags\":(\"text\" \"string\" \"split\" \"separate\" \"parse\") \"description\":\"Takes the provided array, and split_element, and returns an array of arrays which contain the split text strings of the input list.\"}')));
    ...
    },await Environment.do_deferred_splice(await Environment.read_lisp('{\"name\":\"words_and_quotes\" \"fn_args\":\"(text)\" \"description\":\"Given a text string, separates the words and quoted words, returning quoted words as their isolated string.\" \"tags\":(\"text\" \"string\" \"split\" \"separate\" \"parse\") \"usage\":(\"text:string\")}')));
    ...
    },await Environment.do_deferred_splice(await Environment.read_lisp('{\"name\":\"from_style_text\" \"fn_args\":\"(text)\" \"usage\":(\"text:string\") \"description\":\"Given a string or text in the format of an Element style attribute: \\"css_attrib:value; css_attrib2:value\\", split into pairs containing attribute name and value.\" \"tags\":(\"text\" \"css\" \"style\" \"pairs\" \"string\" \"array\" \"list\" \"ui\" \"html\")}')));
    await Environment.set_global("remove_if",async function(f,container) {
        ...
        await (async function() {
         ...
         return  __collector
    },await Environment.do_deferred_splice(await Environment.read_lisp('{\"name\":\"remove_if\" \"fn_args\":\"(f container)\" \"usage\":(\"f:function\" \"container:array\") \"tags\":(\"collections\" \"reduce\" \"filter\" \"where\" \"list\" \"array\" \"reduce\") \"description\":\"Given a function with a single argument, if that function returns true, the value will excluded from the returned array.  Opposite of filter.\"}')));
    await Environment.set_global("filter",async function(f,container) {
        ...
        __action=async function(v) {
            if (check_true (await (async function(){
                let __array_op_rval__79=f;
                 if (__array_op_rval__79 instanceof Function){
                    return await __array_op_rval__79(v) 
                } else {
                    return[__array_op_rval__79,v]
                }
            })())){
                  return v
            }
        ...
        ;
        await (async function() {
            let __for_body__82=async function(__item) {
                __result=await __action(__item);
                if (check_true (__result)){
                      return (__collector).push(__result)
                }
         ...
         return  __collector
    },await Environment.do_deferred_splice(await Environment.read_lisp('{\"name\":\"filter\" \"fn_args\":\"(f container)\" \"usage\":(\"f:function\" \"container:array\") \"tags\":(\"collections\" \"reduce\" \"reject\" \"where\" \"list\" \"array\" \"reduce\") \"description\":\"Given a function with a single argument, if that function returns true, the value will included in the returned array, otherwise it will not.  Opposite of reject.\"}')));
    await Environment.set_global("max_value",async function(v) {
        ...
        await (async function() {
         ...
         return  m
    },await Environment.do_deferred_splice(await Environment.read_lisp('{\"name\":\"max_value\" \"fn_args\":\"(v)\" \"usage\":(\"values:list\") \"description\":\"Given an array of numbers, returns the largest value found.  Any non-numbers in the array are ignored.  If there are no numbers in the list, 0 is returned.\"}')));
    await Environment.set_global("min_value",async function(v) {
        ...
        ;
        await (async function() {
            let __for_body__90=async function(x) {
                if (check_true (await (await Environment.get_global("not"))(await isNaN(x)))){
                      return m=await Math.min(x,m)
                }
            };
            let __array__91=[],__elements__89=v;
            let __BREAK__FLAG__=false;
    ...
    },await Environment.do_deferred_splice(await Environment.read_lisp('{\"name\":\"min_value\" \"fn_args\":\"(v)\" \"usage\":(\"values:list\") \"description\":\"Given an array of numbers, returns the smallest value found.  Any non-numbers in the array are ignored.  If there are no numbers in the list, 0 is returned.\"}')));
    ...
    },await Environment.do_deferred_splice(await Environment.read_lisp('{\"name\":\"tzoffset\" \"fn_args\":\"()\" \"description\":\"Returns the number of seconds the local timezone is offset from GMT\" \"usage\":() \"tags\":(\"time\" \"date\" \"timezone\")}')));
    await Environment.set_global("date_components",async function(date_value,date_formatter) {
        if (check_true (await (await Environment.get_global("is_date?"))(date_value))){
              return await (await Environment.get_global("to_object"))(await (await Environment.get_global("map"))(async function(x) {
                 return  await (async function(){
                    let __array_op_rval__92=(x && x["type"]);
                     if (__array_op_rval__92 instanceof Function){
                        return await __array_op_rval__92((x && x["value"])) 
                    } else {
                        return[__array_op_rval__92,(x && x["value"])]
                    }
                })()
            },await (async function() {
    ...
    },await Environment.do_deferred_splice(await Environment.read_lisp('{\"name\":\"date_components\" \"fn_args\":\"(date_value date_formatter)\" \"usage\":(\"date_value:Date\" \"date_formatter?:DateTimeFormat\") \"description\":\"Given a date value, returns an object containing a the current time information broken down by time component. Optionally pass a Intl.DateTimeFormat object as a second argument.\" \"tags\":(\"date\" \"time\" \"object\" \"component\")}')));
    ...
    await Environment.set_global("add_days",async function(date_obj,num_days) {
        await date_obj["setDate"].call(date_obj,await (await Environment.get_global("add"))(await date_obj["getDate"](),num_days));
         return  date_obj
    },await Environment.do_deferred_splice(await Environment.read_lisp('{\"name\":\"add_days\" \"fn_args\":\"(date_obj num_days)\" \"usage\":(\"date_obj:Date\" \"num_days:number\") \"description\":\"Given a date object and the number of days (either positive or negative) modifies the given date object to the appropriate date value, and returns the date object.\" \"tags\":(\"date\" \"time\" \"duration\" \"days\" \"add\")}')));
    await Environment.set_global("day_of_week",async function(dval) {
         return  await dval["getDay"]()
    },await Environment.do_deferred_splice(await Environment.read_lisp('{\"name\":\"day_of_week\" \"fn_args\":\"(dval)\" \"description\":\"Given a date object, returns the day of the week for that date object\" \"usage\":(\"date:Date\") \"tags\":(\"time\" \"week\" \"date\" \"day\")}')));
    await Environment.set_global("add_hours",async function(date_obj,hours) {
        await date_obj["setHours"].call(date_obj,hours);
         return  date_obj
    },await Environment.do_deferred_splice(await Environment.read_lisp('{\"name\":\"add_hours\" \"fn_args\":\"(date_obj hours)\" \"usage\":(\"date_obj:Date\" \"hours:number\") \"description\":\"Given a date object and the number of hours (either positive or negative) modifies the given date object to the appropriate date value, and returns the date object.\" \"tags\":(\"date\" \"time\" \"duration\" \"hours\" \"add\")}')));
    await Environment.set_global("clear_time",async function(date_obj) {
        await date_obj["setHours"].call(date_obj,0,0,0,0);
         return  date_obj
    },await Environment.do_deferred_splice(await Environment.read_lisp('{\"name\":\"clear_time\" \"fn_args\":\"(date_obj)\" \"usage\":(\"date_obj:Date\") \"description\":\"Given a date object, modifies the date object by clearing the time value and leaving the date value.  Returns the date object.\" \"tags\":(\"date\" \"time\" \"duration\" \"midnight\" \"add\")}')));
    ...
    },await Environment.do_deferred_splice(await Environment.read_lisp('{\"name\":\"yesterday\" \"fn_args\":\"()\" \"description\":\"This function returns an array with two Date values.  The first, in index 0, is the start of the prior day (yesterday midnight), and the second is 24 hours later, i.e. midnight from last night.\" \"usage\":(\"\") \"tags\":(\"time\" \"date\" \"range\" \"prior\" \"hours\" \"24\")}')));
    ...
    },await Environment.do_deferred_splice(await Environment.read_lisp('{\"name\":\"day_before_yesterday\" \"fn_args\":\"()\" \"description\":\"This function returns an array with two Date values.  The first, in index 0, is the start of the day before yesterday (midnight), and the second is 24 later.\" \"usage\":(\"\") \"tags\":(\"time\" \"date\" \"range\" \"prior\" \"hours\" \"24\")}')));
    ...
    },await Environment.do_deferred_splice(await Environment.read_lisp('{\"name\":\"last_week\" \"fn_args\":\"()\" \"description\":\"This function returns an array with two Date values.  The first, in index 0, is the start of the prior week at midnight, and the second is 7 days later, at midnight.\" \"usage\":(\"\") \"tags\":(\"time\" \"date\" \"range\" \"prior\" \"hours\" \"24\")}')));
    ...
    },await Environment.do_deferred_splice(await Environment.read_lisp('{\"name\":\"midnight-to-midnight\" \"fn_args\":\"(dval)\" \"description\":\"This function returns an array with two Date values.  The first, in index 0, is the start of the prior day (yesterday midnight), and the second is 24 hours later, i.e. midnight from last night.\" \"usage\":(\"val:Date\") \"tags\":(\"time\" \"date\" \"range\" \"prior\" \"hours\" \"24\")}')));
        ...
        add_formatter=async function(key,value) {
            await async function(){
                if (check_true( (key==="fractionalSecondDigits"))) {
                    ...
                    await async function(){
                        let __target_obj__95=format_desc;
                        __target_obj__95["hourCycle"]="h24";
        ...
        date_comps=await (await Environment.get_global("date_components"))(date_val,formatter);
         return  (await (async function() {
            let __for_body__103=async function(key) {
                 return  (await (async function(){
                    let __targ__105=date_comps;
                    if (__targ__105){
                         return(__targ__105)[key]
                    } 
                })()||key)
            };
    ...
    await Environment.set_global("set_path_value",async function(root,path,value) {
        if (check_true ((path instanceof Array))){
            ...
            };
            {
                ...
                let parent=await __parent__106();
                ;
    ...
    },await Environment.do_deferred_splice(await Environment.read_lisp('{\"name\":\"set_path_value\" \"fn_args\":\"(root path value)\" \"description\":\"Given an object (the root), a path array, and a value to set, sets the value at the path point in the root object.\" \"usage\":(\"root:object\" \"path:list\" \"value:*\") \"tags\":(\"object\" \"path\" \"resolve\" \"assign\")}')));
    ...
    },await Environment.do_deferred_splice(await Environment.read_lisp('{\"name\":\"has_items?\" \"fn_args\":\"(value)\" \"usage\":(\"value:list\") \"description\":\"Returns true if the list provided has a length greater than one, or false if the list is 0 or nil\" \"tags\":(\"list\" \"values\" \"contains\")}')));
    ...
    },await Environment.do_deferred_splice(await Environment.read_lisp('{\"name\":\"match_all\" \"fn_args\":\"(regex_str search_string)\" \"usage\":(\"regex_str:string\" \"search_string:string\") \"description\":\"Given a regex expression as a string, and the string to search through, returns all matched items via matchAll.\" \"tags\":(\"match\" \"regex\" \"string\" \"find\" \"scan\")}')));
    ...
    },await Environment.do_deferred_splice(await Environment.read_lisp('{\"name\":\"compile_lisp\" \"fn_args\":\"(text)\" \"usage\":(\"text:string\") \"description\":\"Given an input string of lisp text, returns a JSON structure ready for evaluation.\"}')));
    ...
    },await Environment.do_deferred_splice(await Environment.read_lisp('{\"name\":\"has_the_keys?\" \"fn_args\":\"(key_list obj)\" \"usage\":(\"key_list:list\" \"object_to_check:object\") \"description\":\"Given a provided key_list, validate that each listed key or dotted-path-notation value exist in the object.\" \"example\":(((quotem (has_the_keys? (\"type\" \"values.sub_transaction_id\") {\"type\":\"Transaction\" \"group\":\"Receivables\" \"values\":{\"sub_transaction_id\":1242424}})) true))}')));
    ...
    },await Environment.do_deferred_splice(await Environment.read_lisp('{\"name\":\"demarked_number\" \"fn_args\":\"(value separator precision no_show_sign)\" \"usage\":(\"value:number\" \"separator:string\" \"precision:number\" \"no_show_sign:boolean\") \"description\":(+ \"Given a numeric value, a separator string, such as \\",\\" and a precision value \" \"for the fractional-part or mantissa of the value, the demarked_number function will return a string with a formatted value. \" \"Default value for precision is 2 if not provided.\" \"If no_show_sign is true, there will be no negative sign returned, which can be useful for alternative formatting.  See compile_format.\") \"tags\":(\"format\" \"conversion\" \"currency\")}')));
    await Environment.set_global("measure_time",async function(...forms) {
         return  await Environment.do_deferred_splice(await Environment.read_lisp('(let ((end nil) (rval nil) (start (time_in_millis))) (= rval (do \"=$&!\" ' + await Environment.as_lisp ( forms ) + ')) {\"time\":(- (time_in_millis) start) \"result\":rval})'))
    },await Environment.do_deferred_splice(await Environment.read_lisp('{\"eval_when\":{\"compile_time\":true} \"name\":\"measure_time\" \"macro\":true \"fn_args\":\"(\\"&\\" forms)\" \"usage\":(\"form:list\") \"tags\":(\"time\" \"measurement\" \"debug\" \"timing\") \"description\":\"Given a form as input, returns an object containing time taken to evaluate the form in milliseconds with the key time and a result key with the evaluation results.\"}')));
    ...
    },await Environment.do_deferred_splice(await Environment.read_lisp('{\"name\":\"text_to_rgb\" \"fn_args\":\"(rgb_string)\" \"usage\":(\"rgb_string:string\") \"description\":\"Given an RGB hex color string in the form of \\"FFFFFF\\", returns an array containing [ red green blue ] in the set [ 0 1 ].\" \"tags\":(\"colors\" \"graphics\")}')));
    ...
    },await Environment.do_deferred_splice(await Environment.read_lisp('{\"name\":\"tint_rgb\" \"fn_args\":\"(rgb tint_factor)\" \"description\":(+ \"Given an array containing three values between 0 and 1 corresponding to red, \" \"green and blue, apply the provided tint factor to the color and return the result as an rgb array.\" \"The provided tint factor should be in the range 0 (for no tint) to 1 (full tint).\") \"usage\":(\"rgb_value:array\" \"tint_factor:number\") \"tags\":(\"colors\" \"graphics\")}')));
    ...
    },await Environment.do_deferred_splice(await Environment.read_lisp('{\"name\":\"shade_rgb\" \"fn_args\":\"(rgb shade_factor)\" \"description\":(+ \"Given an array containing three values between 0 and 1 corresponding to red, \" \"green and blue, apply the provided tint factor to the color and return the result as an rgb array.\" \"The provided tint factor should be in the range 0 (for no tint) to 1 (full tint).\") \"usage\":(\"rgb_value:array\" \"tint_factor:number\") \"tags\":(\"colors\" \"graphics\")}')));
    ...
    },await Environment.do_deferred_splice(await Environment.read_lisp('{\"name\":\"modify_color_ts\" \"fn_args\":\"(rgb factor)\" \"description\":(+ \"Given an array containing three values between 0 and 1 corresponding to red, \" \"green and blue, apply the provided factor to the color and return the result as an rgb array.\" \"The provided factor should be in the range -1 to 1: -1 to 0 applies shade to the color and 0 to 1 applies tinting to the color.\") \"usage\":(\"rgb_value:array\" \"tint_factor:number\") \"tags\":(\"colors\" \"graphics\")}')));
    await Environment.set_global("is_lower?",async function(v) {
        let c=await v["charCodeAt"].call(v,0);
        ;
         return  ((c>96)&&(c<123))
    },await Environment.do_deferred_splice(await Environment.read_lisp('{\"name\":\"is_lower?\" \"fn_args\":\"(v)\" \"usage\":(\"value:string\") \"description\":\"Given a string as an argument, returns true if the first character of the string is a lowercase character value (ASCII), and false otherwise.\" \"tags\":(\"text\" \"string\" \"lowercase\" \"uppercase\")}')));
    await Environment.set_global("is_upper?",async function(v) {
        let c=await v["charCodeAt"].call(v,0);
        ;
         return  ((c>64)&&(c<91))
    },await Environment.do_deferred_splice(await Environment.read_lisp('{\"name\":\"is_upper?\" \"fn_args\":\"(v)\" \"usage\":(\"value:string\") \"description\":\"Given a string as an argument, returns true if the first character of the string is an uppercase character value (ASCII), and false otherwise.\" \"tags\":(\"text\" \"string\" \"lowercase\" \"uppercase\")}')));
    ...
    await Environment.set_global("scan_list",async function(regex,container) {
        ...
        await (async function() {
         ...
         return  results
    },await Environment.do_deferred_splice(await Environment.read_lisp('{\"name\":\"scan_list\" \"fn_args\":\"(regex container)\" \"description\":(+ \"Scans a list for the provided regex expression and returns the indexes in the list where it is found.  \" \"The provided regex expression can be a plain string or a RegExp object.\") \"usage\":(\"regex:string\" \"container:list\") \"tags\":(\"search\" \"index\" \"list\" \"regex\" \"array\" \"string\")}')));
    ...
    },await Environment.do_deferred_splice(await Environment.read_lisp('{\"name\":\"squeeze\" \"fn_args\":\"(s)\" \"usage\":(\"string_value:string\") \"description\":\"Returns a string that has all spaces removed from the supplied string value.\" \"tags\":(\"text\" \"space\" \"trim\" \"remove\")}')));
